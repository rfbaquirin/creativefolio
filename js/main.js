...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-pdf]').forEach(el => {
    el.style.cursor = 'pointer';
    const handlePdf = () => {
      // Mobile browsers' built-in PDF viewers don't respect the lightbox's
      // width, causing the page to render oversized and cut off. Full-screen
      // in a new tab is both more reliable and a better reading experience
      // for a multi-page document on a small screen.
      if (window.innerWidth < 700) {
        window.open(el.dataset.pdf, '_blank');
      } else {
        openPdfLightbox(el.dataset.pdf);
      }
    };
    el.addEventListener('click', handlePdf);
    el.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        handlePdf();
